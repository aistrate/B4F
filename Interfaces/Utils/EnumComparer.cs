...
namespace B4F.TotalGiro.Utils
{
        ...
        where TEnum : struct, IComparable, IConvertible, IFormattable
    {
        ...
        private static readonly Func<TEnum, TEnum, TEnum> bitwiseAnd;
        private static readonly Func<int, TEnum, TEnum> bitwiseAnd2;
        private static readonly Func<TEnum, int> getHashCode;
        //private static readonly Func<TEnum, TEnum[]> toArray;
        ...
        static EnumComparer()
        {
            ...
            bitwiseAnd = generateBitwiseAnd();
            bitwiseAnd2 = generateBitwiseAnd2();
        ...
        public bool ContainsValue(TEnum x, TEnum y)
        {
            return equals(bitwiseAnd(x,y),y);
        }

        public bool ContainsValue(int x, TEnum y)
        {
            return equals(bitwiseAnd2(x, y), y);
        }
        ...
        private static Func<TEnum, TEnum, TEnum> generateBitwiseAnd()
        {
            var xParam = Expression.Parameter(typeof(TEnum), "x");
            var yParam = Expression.Parameter(typeof(TEnum), "y");
            var bitwiseAndExpression = Expression.Convert( Expression.And(Expression.Convert(xParam, typeof(int)), Expression.Convert(yParam, typeof(int))), typeof(TEnum));
            return Expression.Lambda<Func<TEnum, TEnum, TEnum>>(bitwiseAndExpression, new[] 
						        { xParam, yParam }).Compile();
        }

        private static Func<int, TEnum, TEnum> generateBitwiseAnd2()
        {
            var xParam = Expression.Parameter(typeof(int), "x");
            var yParam = Expression.Parameter(typeof(TEnum), "y");
            var bitwiseAndExpression = Expression.Convert( Expression.And(xParam, Expression.Convert(yParam, typeof(int))), typeof(TEnum));
            return Expression.Lambda<Func<int, TEnum, TEnum>>(bitwiseAndExpression, new[] 
						        { xParam, yParam }).Compile();
        }
        ...
        //private static Func<TEnum, TEnum[]> generateToArray()
        //{
        //    var objParam = Expression.Parameter(typeof(TEnum), "obj");
        //    var underlyingType = Enum.GetUnderlyingType(typeof(TEnum));
        //    var convertExpression = Expression.Convert(objParam, underlyingType);
